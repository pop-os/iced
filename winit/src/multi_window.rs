    ...
    A::Message: Send + 'static,
{
    ...
    if let Some(stream) = task.into_stream() {
        runtime.run(Box::pin(stream.map(|s| UserEventWrapper::Message(s))));
    }
    ...
    impl<Message, F, C>
        winit::application::ApplicationHandler<
            UserEventWrapper<Action<Message>>,
        ...
        C: Compositor,
    {
        ...
        fn user_event(
            &mut self,
            event_loop: &winit::event_loop::ActiveEventLoop,
            message: UserEventWrapper<Action<Message>>,
        ) {
            self.process_event(
                event_loop,
...
enum Event<Message: 'static> {
    ...
    EventLoopAwakened(winit::event::Event<UserEventWrapper<Action<Message>>>),
}
...
async fn run_instance<A, E, C>(
    ...
    mut runtime: Runtime<
        E,
        Proxy<UserEventWrapper<Action<A::Message>>>,
        UserEventWrapper<Action<A::Message>>,
    >,
    mut proxy: Proxy<UserEventWrapper<Action<A::Message>>>,
    mut debug: Debug,
    ...
    A::Message: Send + 'static,
{
    ...
    let mut clipboard =
        Clipboard::connect(&main_window.raw, Proxy::new(proxy.raw.clone()).0);
    ...
    runtime.track(
        application
            .subscription()
            .map(|s| UserEventWrapper::Message(Action::Output(s)))
            .into_recipes(),
    );
            ...
            Event::EventLoopAwakened(event) => {
                match event {
                    ...
                    event::Event::UserEvent(message) => {
                        match message {
                            UserEventWrapper::Message(action) => {
                                run_action(
                                    action,
                                    &application,
                                    &mut compositor,
                                ...
                                let Some(window_id) =
                                    source_surface.and_then(|source| {
                                        ...
                                        core::clipboard::DndSource::Widget(
                                            ...
                                            let operation: Box<dyn Operation<()>> = Box::new(
                                                operation::map(Box::new(search_id::search_id(w.clone())), |_| {}));
                            ...
                            // TODO cleanup
                            UserEventWrapper::Dnd(_) => {} // UserEventWrapper::Dnd(e) => match &e {
                                                           //     dnd::DndEvent::Offer(
                                                           //         _,
                                                           ...
                                                           //         events.push((id, core::Event::Dnd(e)))
                                                           //     }
                                                           // },
                        };
                    }
    ...
    match action {
        ...
        Action::Dnd(a) => match a {
            ...
            iced_runtime::dnd::DndAction::StartDnd {
                ...
                internal,
                source_surface,
                icon_surface.map(|s| s as Box<dyn Any>),
                content,
                actions,
            ),
